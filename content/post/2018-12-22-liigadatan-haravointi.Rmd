...
Etuna excel copypastemetodiin on kuitenkin se, että saman voi tehdä yht'aikaisesti jokaiselle kaudelle, mitä liigan sivustolta löytyy:
```{r eval=TRUE, message=FALSE, warning=FALSE, echo = TRUE}
...
liigadata 
```
Datassa on kaikki pelit kaudelta 1975-1976 haravointihetkeen asti. Data ei ole vielä aivan optimaalisessa muodossa kaikkea laskentaa koskkien, mutta muodoltaan yksi rivi per ottelu on varmasti jokaiselle jääkiekkoharrastajalle tuttu. Pesäistettynä data on ymmärrettävässä käyttömuodossa seuraavia laskentoja varten.

```{r, eval = T, echo = T}
liigadata %>% 
  nest(-kausi) %>% 
  saveRDS('liigadata.rds')
```
